...
class PredictionComparison(eqx.Module):
    ...
    def __init__(self, grid, action_dim, obs_dim):
        assert (
            grid.shape[-1] == (action_dim + obs_dim).astype(int)
...
class Evaluator(eqx.Module):
    ...
    def __init__(
            self, constraint_function, gt_model, obs_dim, act_dim, validation_points_per_dim):
        self.constraint_function = constraint_function
        self.constraint_data_space_grid = valid_space_grid(
            constraint_function, obs_dim+act_dim, points_per_dim, -1, 1)
