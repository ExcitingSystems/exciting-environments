...
class FastPMSM:
    ...
    t32 = np.array([[1, 0], [-0.5, 0.5 * np.sqrt(3)], [-0.5, -0.5 * np.sqrt(3)]])  # only for alpha/beta -> abc
    t23 = (
        2 / 3 * np.array([[1, 0], [-0.5, 0.5 * np.sqrt(3)], [-0.5, -0.5 * np.sqrt(3)]]).T  # only for abc -> alpha/beta
    )
    tau = 1e-4
    i_limit = 250  # in A
    u_limit = 266.666667  # u_DC/2 in V
    ...
    def ode_step(self, u_dq, i_dq, l_dq, r_s, omega, psi_p):
        if self.saturated:
            ...
            # print(self.eps)
            p_d = {q: interp(i_dq) for q, interp in self.interpolators.items()}
            L_diff = np.column_stack([p_d[q] for q in ["L_dd", "L_dq", "L_qd", "L_qq"]]).reshape(-1, 2, 2)
            L_diff_inv = np.linalg.inv(L_diff)
            psi_dq = np.column_stack([p_d[psi] for psi in ["Psi_d", "Psi_q"]])
            di_dq_1 = np.einsum(
                "bij,bj->bi", (np.eye(2, 2)[np.newaxis, ...] - L_diff_inv * self.r_s_const * self.tau), i_dq
            )
            di_dq_2 = np.einsum("bij,bjk,bk->bi", L_diff_inv, q, u_dq * self.tau)
            di_dq_3 = np.einsum("bij,bjk,bk->bi", L_diff_inv, (q - np.eye(2, 2)[np.newaxis, ...]), psi_dq)
            i_dq_k1 = di_dq_1 + di_dq_2 + di_dq_3

            # debug
            if self.debug:
                self.di_dq_1 += [di_dq_1]
                self.di_dq_2 += [di_dq_2]
                self.di_dq_3 += [di_dq_3]
                self.A_mat += [-L_diff_inv * q * self.r_s_const / self.i_limit]
        else:
            # fundamental wave model
            # Source: https://doi.org/10.1109/IEMDC.2019.8785122
            q = np.array([[-r_s, omega * l_dq[:, 1]], [-omega * (l_dq[:, 0]), -r_s]])
            di_dq = (
                u_dq
                + np.einsum("ijb,bj->bi", q, i_dq)
                - omega.reshape(-1, 1) * np.column_stack([np.zeros_like(psi_p), psi_p])
            ) / l_dq
    ...
    def step(self, u_dq_norm):
        ...
        self.u_albet_norm = self.dq2albet(u_dq_norm, self.get_advanced_angle(self.eps, 0.5, self.tau, self.me_omega))
        ...
        # ode
        self.i_dq = self.ode_step(u_dq, self.i_dq, self.l_dq, self.r_s, self.el_omega, self.psi_p)
    ...
    def reset(self, shuffle_references=True, random_initial_values=False):
        ...
        if random_initial_values:
            # left i_dq half plane
            ref_pointers = np.column_stack(
                [np.random.rand(self.batch_size) * 0.99, (np.random.rand(self.batch_size) * np.pi + np.pi / 2)]
            )
            # polar -> cartesian
            self.i_dq = np.column_stack(
                [ref_pointers[:, 0] * np.cos(ref_pointers[:, 1]), ref_pointers[:, 0] * np.sin(ref_pointers[:, 1])]
            ).astype(np.float32)
        else:
        ...
        if shuffle_references:
            ...
            self.idx_sample = np.array(random.sample(range(500, self.x_star.shape[1] - 500), self.batch_size))
