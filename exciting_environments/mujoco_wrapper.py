...
qpos_names_type={
    "0":["body_position_x","body_position_y","body_position_z","body_orientation_qw","body_orientation_qx","body_orientation_qy","body_orientation_qz"],
    "1":["ball_orientation_qw", "ball_orientation_qx", "ball_orientation_qy", "ball_orientation_qz"],
    "2":["position"],
    "3":["angle"]}
qvel_names_type={
    "0":["body_linear_velocity_x", "body_linear_velocity_y", "body_linear_velocity_z","body_angular_velocity_x", "body_angular_velocity_y", "body_angular_velocity_z"],
...
class MujucoWrapper(ABC):
    ...
    def __init__(
        ...
        Args:
            ...
            tau (float): Duration of one control step in seconds.
        ...
        action_names = [mujoco.mj_id2name(mujoco_model, mujoco.mjtObj.mjOBJ_ACTUATOR, i) for i in range(mujoco_model.nu)]
        ...
        self.env_properties=self.EnvProperties(physical_normalizations=phys_norm,action_normalizations=action_normalization,static_params=None)

        self.action_description=action_names
        self.obs_description=list(self.env_properties.physical_normalizations.qpos.__dict__.keys())+list(self.env_properties.physical_normalizations.qvel.__dict__.keys())
    ...
    def generate_physical_normalization_dataclasses(self,model):
        ...
        for i in range(model.njnt):
            ...
            q_pos.update({
                name: (MinMaxNormalization(min=-jnp.pi, max=jnp.pi) if angle_flags[i] == 1 
                    else MinMaxNormalization(min=jnp.nan, max=jnp.nan)) if joint.limited[0] == 0 
    ...
    def generate_action_normalization_dataclasses(self,model):
        ...
        action_normalization_data = {name: MinMaxNormalization(min=jnp.nan, max=jnp.nan) if action_limited[i]==0 else MinMaxNormalization(min=action_ranges[i, 0], max=action_ranges[i, 1]) for i, name in enumerate(action_names)}
    ...
    def generate_state_from_observation(self, obs, env_properties, key=None):
        ...
        """
        # will see how state is defined and if observation and state even deviate
        return
    ...
    def normalize_components(self, array, normalizations):
        for i,field in enumerate(fields(normalizations)):
            name = field.name
            norm_value = getattr(normalizations, name).normalize(
                array[i]
            )
    ...
    def denormalize_components(self, array, normalizations):
        for i,field in enumerate(fields(normalizations)):
            name = field.name
            denorm_values = getattr(normalizations, name).denormalize(
                array[i]
            )
