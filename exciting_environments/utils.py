...
def dump_sim_properties_to_json(params, action_normalizations, physical_normalizations, tau, filename):
    action_norm_serialized = {k: asdict(v) for k, v in action_normalizations.items()}
    physical_norm_serialized = {k: asdict(v) for k, v in physical_normalizations.items()}
    data = {
        "params": params,
...
def load_sim_properties_from_json(filename):
    ...
    tau = data["tau"]
    action_normalizations = {key: MinMaxNormalization(**value) for key, value in action_norm_serialized.items()}
    physical_normaliztions = {key: MinMaxNormalization(**value) for key, value in physical_norm_serialized.items()}
