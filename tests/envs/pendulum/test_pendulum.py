...
def test_default_initialization():
    ...
    params = {"g": jnp.array(9.81), "l": jnp.array(2), "m": jnp.array(1)}
...
def test_custom_initialization():
    ...
    physical_normalizations = {
        "theta": MinMaxNormalization(min=jnp.array(-jnp.pi / 2), max=jnp.array(jnp.pi / 2)),
        "omega": MinMaxNormalization(min=jnp.array(-5), max=jnp.array(3)),
    }
    action_normalizations = {"torque": MinMaxNormalization(min=jnp.array(-10), max=jnp.array(10))}
    params = {"l": jnp.array(1), "g": jnp.array(9.81), "m": jnp.array(1)}
    env = excenvs.make(
        "Pendulum-v0",
