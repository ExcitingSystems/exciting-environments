...
def test_default_initialization(motor_variant):
    ...
    env = EnvironmentRegistry.IM.make(motor_variant=motor_variant)
...
def test_custom_initialization():
    ...
    physical_normalizations = {
        "u_alpha_buffer": MinMaxNormalization(min=(-2 * 450 / 3), max=(2 * 26 / 3)),
        "u_beta_buffer": MinMaxNormalization(min=(-2 * 320 / 3), max=(2 * 300 / 3)),
        "epsilon": MinMaxNormalization(min=jnp.repeat((-jnp.pi / 2), batch_size), max=(jnp.pi)),
        "i_s_alpha": MinMaxNormalization(min=(-7), max=(0)),
        "i_s_beta": MinMaxNormalization(min=(-6), max=(12)),
        "psi_r_alpha": MinMaxNormalization(min=(-2.5), max=(0.55)),
        "psi_r_beta": MinMaxNormalization(min=(-0.5), max=(1.6)),
    ...
    action_normalizations = {
        "u_alpha": MinMaxNormalization(min=(-2 * 350 / 3), max=(2 * 26 / 3)),
        "u_beta": MinMaxNormalization(min=(-2 * 320 / 3), max=(2 * 300 / 3)),
    }
    params = {
    ...
    env = EnvironmentRegistry.IM.make(
